  ...
  try {
    ...
    switch (pathname) {
      case "/shipping/pg_req_test1.aspx": {
        ...
        // let genderColumn = table
        //   .querySelectorAll("tr")[7]
        //   .querySelectorAll("td")[3];
        // let select = genderColumn.querySelectorAll("select")[0];
        // let option = select.querySelectorAll("option")[1];
        // if (option) {
        ...
        for (let i = 7; i < rows.length; i++) {
          ...
          if (i === 7) {
            if (name) {
              // get the input type text tag and change its value
              name.querySelectorAll("input")[0].value = "Rafique";
            }
          }
          if (i === 8) {
            if (name) {
              // get the input type text tag and change its value
              name.querySelectorAll("input")[0].value = "Furqan";
            }
          }

          if (gender) {
            let option = gender.querySelectorAll("option")[1];
            if (option) {
              option.selected = true;
            }
          }
          if (category) {
            let option = category.querySelectorAll("option")[2];
            if (option) {
              option.selected = true;
            }
          }
